...
function LldSidebar({
  ...
  // Calculate total topics & total weeks metrics
  const totalTopics = sections.reduce((acc, sec) => acc + sec.topics.length, 0);
  const totalWeeks = sections.length;
  ...
  // Completed weeks count (week is completed if all its topics are checked)
  const completedWeeksCount = sections.filter((sec) =>
  ...
  const weekOffset = CIRCUMFERENCE - (weekPercent / 100) * CIRCUMFERENCE;

  return (
    <aside className="xlr-hld-sidebar">
      {/* Sidebar Header */}
      <div className="xlr-hld-sidebar-header">
        <h3 className="xlr-hld-sidebar-title">MASTER LOW LEVEL DESIGN</h3>
        <p className="xlr-hld-sidebar-subtitle">0 skippable topic · covered 100%</p>
      </div>

      {/* Aesthetic Compact Progress Rings Section matching reference screenshots */}
      <div className="xlr-hld-rings-container">
        ...
        <div className="xlr-hld-ring-item">
          <div className="xlr-hld-ring-wrapper">
            ...
            <div className="xlr-hld-ring-text-container">
              <span className="xlr-hld-ring-percent">{weekPercent}%</span>
              <span className="xlr-hld-ring-fraction">{completedWeeksCount}/{totalWeeks}</span>
            </div>
          </div>
          <span className="xlr-hld-ring-label">WEEKS</span>
        </div>
      </div>

      {/* 16 Weeks Scrollable List */}
